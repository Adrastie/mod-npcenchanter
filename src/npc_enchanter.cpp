...
public:
    ...
    void OnLogin(Player* player)
    {
        ...
        {
            ChatHandler(player->GetSession()).SendSysMessage("This server is running the |cff4CFF00RE-EnchanterNPC |rmodule.");
        }
    }
};
...
public:
    ...
    void Enchant(Player* player, Creature* creature, Item* item, uint32 enchantid)
    {
        if (!item)
        {
            ...
            creature->MonsterWhisper("You must equip the item you would like to enchant!", player, 0);
            player->PlayerTalkClass->SendCloseGossip();
            return;
        }
        ...
        if (!enchantid)
        {
            player->GetSession()->SendNotification("Something went wrong. It has been logged for developers and will be looked into, sorry for the inconvenience.");
            player->PlayerTalkClass->SendCloseGossip();
            //creature->HandleEmoteCommand(EMOTE_ONESHOT_LAUGH);
            return;
        }
